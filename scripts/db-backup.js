...
const targetPath = getOptionalPathArg();

app.whenReady().then(() => {
  try {
    const result = storage.backupDatabase(targetPath);
    console.log(`SQLite backup created at ${result.backupPath}`);
  } catch (e) {
    console.error("SQLite backup failed:", e && e.message ? e.message : e);
    process.exitCode = 1;
  } finally {
