...
function isProbablyHeaderRow(row) {
  ...
  for (let i = 0; i < row.length; i++) {
    ...
    if ((v === "id" || v.endsWith(" id") || v.endsWith(" title")) || v === "title" || v === "name" || v === "hardware" || v === "rate") return true;
  }
  return false;
}
...
function previewImportHardwares() {
...
function importHardwaresFromText(text) {
  ...
  return Promise.all([
    ...
    usable.forEach((row) => {
      ...
      let title = "";
      if (headerIndex) {
        const titleIdx = headerIndex.title != null ? headerIndex.title : headerIndex.name != null ? headerIndex.name : headerIndex.hardware_title != null ? headerIndex.hardware_title : headerIndex.hardware != null ? headerIndex.hardware : null;
      ...
      const drawerHandlesIdx =
        headerIndex && headerIndex.drawer_handles != null
          ...
          : headerIndex && headerIndex.drawer_handle != null
            ...
            : headerIndex && headerIndex.internal_handle != null
              ? headerIndex.internal_handle
              : null;
...
if (document.getElementById("import-text")) {
  document.getElementById("import-text").addEventListener("input", previewImportHardwares);
}
