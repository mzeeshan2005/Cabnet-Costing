...
function isProbablyHeaderRow(row) {
  ...
  for (let i = 0; i < row.length; i++) {
    ...
    if (v === "id" || v === "title" || v === "name" || v === "handle" || v === "handler" || v === "rate") return true;
  }
  return false;
}
...
function importHandlersFromText(text) {
  ...
  return Promise.all([
    ...
    usable.forEach((row) => {
      ...
      let title = "";
      if (headerIndex) {
        const titleIdx = headerIndex.title != null ? headerIndex.title : headerIndex.name != null ? headerIndex.name : headerIndex.handle != null ? headerIndex.handle : headerIndex.handler != null ? headerIndex.handler : null;
