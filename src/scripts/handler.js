...
function isProbablyHeaderRow(row) {
  ...
  for (let i = 0; i < row.length; i++) {
    ...
    if ((v === "id" || v.endsWith(" id") || v.endsWith(" title")) || v === "title" || v === "name" || v === "handle" || v === "handler" || v === "rate") return true;
  }
  return false;
}
...
function previewImportHandlers() {
...
function importHandlersFromText(text) {
  ...
  return Promise.all([
    ...
    usable.forEach((row) => {
      ...
      let title = "";
      if (headerIndex) {
        const titleIdx = headerIndex.title != null ? headerIndex.title : headerIndex.name != null ? headerIndex.name : headerIndex.handle_title != null ? headerIndex.handle_title : headerIndex.handle != null ? headerIndex.handle : headerIndex.handler != null ? headerIndex.handler : null;
...
if (file_manager && typeof file_manager.bindExcelImportControls === "function") {
  excelImport = file_manager.bindExcelImportControls({ preferredSheetName: "Handles", afterTextSet: previewImportHandlers, fileNameDisplayId: "import-file-name" });
}
...
if (document.getElementById("import-open")) {
  document.getElementById("import-open").addEventListener("click", (event) => {
    ...
    if ((!excelImport || excelImport.isBound === false) && file_manager && typeof file_manager.bindExcelImportControls === "function") {
      excelImport = file_manager.bindExcelImportControls({ preferredSheetName: "Handles", afterTextSet: previewImportHandlers });
    }
...
if (document.getElementById("import-text")) {
  document.getElementById("import-text").addEventListener("input", previewImportHandlers);
}
