...
function clearFields() {
  ...
  document.getElementById("pin").value = "0";
  file_manager
    ...
    .map(
      (data) => {
        ...
        <tr class="tr-shadow" style="border-bottom: 2px solid grey${isNew ? '; background-color: #d4edda' : ''}">
          ...
          <td style="border: 1px solid black">${data.pin}</td>
...
function currentShelfQuery() {
  const el = document.getElementById("search");
  return el && el.value != null ? String(el.value) : "";
}

function currentShelfFilters() {
  return {
...
function listDataShelvesFiltered(query, utility_id, type_id, code_id) {
...
function refreshShelvesTable() {
  const query = currentShelfQuery();
  const f = currentShelfFilters();
  return file_manager
    .searchShelves(query, f.utility_id, f.type_id, f.code_id, 800)
    .then((rows) => {
      const merged = mergeById(rows || [], listDataShelvesFiltered(query, f.utility_id, f.type_id, f.code_id));
      renderShelvesTable(merged);
    });
}

let shelfSearchTimer = null;
function scheduleRefreshShelvesTable() {
  if (shelfSearchTimer) clearTimeout(shelfSearchTimer);
  shelfSearchTimer = setTimeout(() => {
    refreshShelvesTable();
  }, 150);
}
...
function isProbablyHeaderRow(row) {
  ...
  for (let i = 0; i < row.length; i++) {
    ...
    if ((v === "id" || v.endsWith(" id") || v.endsWith(" title")) || v === "title" || v === "name" || v === "shelf" || v === "shelve" || v === "rate") return true;
  }
  return false;
}
...
function importShelvesFromText(text) {
  ...
  return Promise.all([
    ...
    const existingByKey = {};
    existing.forEach((s) => {
      const n = s && s.id != null ? Number(s.id) : 0;
      if (n > 0) usedIds.add(n);
      if (s && s.utility_id != null && s.code_id != null && s.title) {
        existingByKey[String(s.utility_id) + "::" + (s.type_id != null ? String(s.type_id) : "") + "::" + String(s.code_id) + "::" + String(s.title).toLowerCase().trim() + "::" + (s.rate != null ? String(s.rate) : "") + "::" + (s.pin != null ? String(s.pin) : "") + "::" + (s.edging != null ? String(s.edging) : "")] = s.id;
      }
    });
    listData.forEach((s) => {
      const n = s && s.id != null ? Number(s.id) : 0;
    ...
    usable.forEach((row) => {
      ...
      let title = "";
      if (headerIndex) {
        const titleIdx = headerIndex.title != null ? headerIndex.title : headerIndex.name != null ? headerIndex.name : headerIndex.shelf_title != null ? headerIndex.shelf_title : headerIndex.shelf != null ? headerIndex.shelf : headerIndex.shelve != null ? headerIndex.shelve : null;
      ...
      const rateIdx = headerIndex && headerIndex.rate != null ? headerIndex.rate : headerIndex && headerIndex.shelve_area != null ? headerIndex.shelve_area : null;
      const pinIdx = headerIndex && headerIndex.pin != null ? headerIndex.pin : headerIndex && headerIndex.pin_qty != null ? headerIndex.pin_qty : null;
      ...
      const dupKey = String(utilityId) + "::" + String(typeId) + "::" + String(codeId) + "::" + title.toLowerCase().trim() + "::" + String(rate) + "::" + String(pin) + "::" + String(edging);
      importRowOrder = importRowOrder || [];
      if (existingByKey[dupKey]) {
        importRowOrder.push(String(existingByKey[dupKey]));
        return;
      }
...
if (file_manager && typeof file_manager.bindExcelImportControls === "function") {
  excelImport = file_manager.bindExcelImportControls({ preferredSheetName: "Adjustable Shelves", afterTextSet: previewImportShelves, fileNameDisplayId: "import-file-name" });
}
        ...
        if (res[0].password === document.getElementById("pass").value){
          file_manager
              ...
              .then((res) => {
                ...
                file_manager
                    .writeFile(
                        path.join(__dirname, "../../db/.shelves.json"),
                        clients
                    )
                    .then((res) => {
                      if (res === "success") {
