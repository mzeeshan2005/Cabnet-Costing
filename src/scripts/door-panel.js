...
function populateTable() {
  ...
  document.getElementById("update").disabled = true;
  document.getElementById("save").disabled = true;
  file_manager.nextId(".doors.json").then((nextId) => {
...
function isProbablyHeaderRow(row) {
  ...
  for (let i = 0; i < row.length; i++) {
    ...
    if (v === "id" || v === "title" || v === "name" || v === "finishing" || v === "door" || v === "rate") return true;
  }
  return false;
}
...
function importDoorsFromText(text) {
  ...
  return Promise.all([
    ...
    existing.forEach((d) => {
      ...
      const u = d && d.utility_id != null ? String(d.utility_id) : "";
      const t = d && d.type_id != null ? String(d.type_id) : "";
      const c = d && d.code_id != null ? String(d.code_id) : "";
      const title = d && d.title != null ? String(d.title).trim().toLowerCase() : "";
    ...
    usable.forEach((row) => {
      ...
      let title = "";
      if (headerIndex) {
        const titleIdx = headerIndex.title != null ? headerIndex.title : headerIndex.name != null ? headerIndex.name : headerIndex.finishing != null ? headerIndex.finishing : headerIndex.door != null ? headerIndex.door : null;
